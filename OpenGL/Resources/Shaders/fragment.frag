...
vec3 GetPointLights(vec3 textureColor)
{
	...
	for (uint i = 0u; i < PLightsCount; i++)
	{
		...
		float deltaB = pow(max(dot(normal, halfVV), 0.0), lsmoothness * MatSmoothness);
...
{
	if (SLightsCount == 0u) return vec3(0,0,0);
	vec3 amb;
	vec3 dif;
	vec3 spe;
...
{
	if (DLightsCount == 0u) return vec3(0,0,0);
	vec3 amb;
	vec3 dif;
	vec3 spe;
	for (uint i = 0u; i < DLightsCount; i++)
	{
		...
		float deltaB = pow(max(dot(normal, halfV), 0.0), lsmoothness * MatSmoothness);
		amb = amb + vec3(lambient * MatAmbient);
		dif = dif + vec3(ldiffuse*MatDiffuse*deltaA);
		spe = spe + vec3(MatSpecular*lspecular*deltaB);
	}
	spe = spe * MatShininess;
	return textureColor * (amb+dif+spe) + (1-MatAbsorbtion) * spe;
}
...
void main()
{
	...
	FragColor = vec4(GetDirectionalLights(texRGB) + GetPointLights(texRGB) + GetSpotLights(texRGB) + texRGB*MatEmissive, texColor.w*MatAlpha);
}